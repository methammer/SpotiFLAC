...
export function WatchlistPage() {
  ...
  const handleEditSave = async () => {
    if (!editingId) return;
    try {
      await UpdateWatchlist({ id: editingId, interval_hours: parseInt(editInterval, 10), sync_deletions: editSyncDeletions });
    ...
    <div className="space-y-6">
      ...
      {watchlists.length > 0 && (
        <div className="bg-primary/10 border border-primary/20 p-3 rounded-md flex items-center gap-3 text-sm text-primary">
          ...
          <p>
            Auto-Sync uses your current <strong>Settings</strong> (destination folder, quality, lyrics) saved at the time you add the playlist.
          </p>
        </div>
      )}
        ...
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-4 py-2">
            ...
            <div className="border rounded-md p-3 bg-muted/30">
              <div className="flex items-center gap-2">
                <input type="checkbox" id="edit-sync-deletions" checked={editSyncDeletions}
                  onChange={(e) => setEditSyncDeletions(e.target.checked)} className="rounded" />
                <label htmlFor="edit-sync-deletions" className="text-sm cursor-pointer">
      ...
      <Dialog open={isAddModalOpen} onOpenChange={setIsAddModalOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              SpotiFLAC will periodically check this playlist for new tracks and download them automatically.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-2">
            <div className="space-y-2">
            ...
            <div className="space-y-2 border rounded-md p-3 bg-muted/30">
              ...
              <div className="flex items-center gap-2">
                <input type="checkbox" id="sync-deletions" checked={newSyncDeletions}
                  onChange={(e) => setNewSyncDeletions(e.target.checked)} className="rounded" />
                <label htmlFor="sync-deletions" className="text-sm cursor-pointer">
                  Sync deletions <span className="text-xs text-muted-foreground">(delete file if removed from Spotify)</span>
                </label>
              </div>
            </div>
